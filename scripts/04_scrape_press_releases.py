...
def find_press_release_exhibit(items: list[dict]) -> dict | None:
    """Pick the best ex99.1 candidate from a filing directory listing."""
    best = None
    for item in items:
        name = item.get("name", "") or ""
        if name.lower().endswith(IGNORE_SUFFIXES):
            continue
        if "index" in name.lower():
            continue
        if EX99_1_RE.search(name):
            # Prefer HTM/HTML over plain, and largest file
            size = int(item.get("size", 0) or 0)
            score = (1 if name.lower().endswith((".htm", ".html")) else 0, size)
...
def process_firm(row: pd.Series, cik: str, start_year: int, end_year: int,
    ...
    def process_filing_list(forms_, dates_, accs_, items_, saved):
        for i, form in enumerate(forms_):
            ...
            directory_items = idx.get("directory", {}).get("item", [])
            exhibit = find_press_release_exhibit(directory_items)
            if not exhibit:
                continue
            ...
            index_rows.append({
                ...
                "size_bytes": int(exhibit.get("size", 0) or 0),
            })
            saved[0] += 1
